...
- Backend: FastAPI Python API with RAG, OCR, document ingestion, local Ollama LLM, and ChromaDB vector storage
...
- Local Ollama API through `requests`
...
If `winget` is unavailable, install them manually from:
...
After installing Tesseract, make sure this folder is available in the Windows `PATH`:
...
```powershell
cd C:\path\to\EduBot_RAG_edited\frontend
npm install
copy .env.example .env
```
...
#### 5. Quick Windows Verification

With the backend virtual environment active:

```powershell
python -c "from llm import generate; print(generate('Say hello in one sentence.'))"
```

Then open the frontend and send a test chat message. The first backend run may take extra time while the embedding model downloads.
...
7. Chunks are embedded using `BAAI/bge-small-en-v1.5`.
...
7. Context is sent to local Ollama.
8. Ollama returns a document-only answer.
